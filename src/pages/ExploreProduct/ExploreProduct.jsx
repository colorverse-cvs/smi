...
export default function ExploreProduct() {
  ...
  useEffect(() => {
    ...
    if (!category) {
      ...
      products = Data.product_catalog.filter(
  ...
  }, [category, selectedSubCategory, search, selectedCertifications]);
    ...
    <>
      <div className="w-full px-32 py-10">
          ...
          <div className="flex flex-col w-full md:max-w-[40%]">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                ...
                className={`px-4 py-1 rounded-full border text-sm transition ${selectedSubCategory === ""
              ...
              {uniqueSubCategories.map((subCat) => (
                <button
                  ...
                  className={`px-4 py-1 rounded-full border text-sm transition ${selectedSubCategory === subCat
                    ? "bg-[#077CEB] text-white border-[#077CEB]"
                    : "bg-gray-100 text-gray-700 hover:bg-blue-50 hover:border-[#077CEB]"
                    }`}
                >
        ...
        <div className="flex flex-col md:flex-row gap-6">
          <aside className="hidden w-full md:w-1/5 bg-gray-50 border-gray-50 rounded-lg p-5">
            ...
            <div>
              ...
              {["ISO", "CE", "ROHS"].map((c) => (
                <div key={c} className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={selectedCertifications.includes(c)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedCertifications([
                        ...
                        setSelectedCertifications(
                          selectedCertifications.filter((cert) => cert !== c)
                        );
                      }
                    }}
                  />
            ...
            {currentProducts.length === 0 ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
                {currentProducts.map((p, index) => (
                  <div
                    key={index}
                    className="border border-transparent rounded-xl bg-white shadow-sm hover:shadow-lg hover:border-[#077CEB] transition-all duration-200 p-4 flex flex-col items-center justify-between"
                  >
                    ...
                    <button
                      ...
                      className="w-full py-2 text-sm text-[#077CEB] border border-[#077CEB] rounded-md hover:bg-[#077CEB] hover:text-white cursor-pointer"
                    >
                      View Details
                    </button>
                  </div>
          ...
          <div className="flex justify-center items-center gap-3 mt-10">
            ...
            {Array.from({ length: totalPages }, (_, i) => i + 1).map((n) => (
              <button
                ...
                className={`w-8 h-8 flex items-center justify-center rounded-full transition ${n === currentPage
